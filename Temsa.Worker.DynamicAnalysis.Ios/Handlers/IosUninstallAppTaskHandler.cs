...
public class IosUninstallAppTaskHandler(
    IIosUninstallAppExecutor executor,
    ...
    ILogger<IosInstallAppTaskHandler> logger) : IWorkerTaskHandler
{
    ...
    private readonly IIosUninstallAppExecutor _executor = executor;
    ...
    public string TaskType => "ios-uninstall-app";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new IosUninstallAppTaskParameters(
            InputArtifact: context.Task.InputArtifact,
            BundleId: pipelineParameters.BundleId);
        
        _logger.LogInformation(
            "Executing iOS uninstall app task {ScanTaskId}. BundleId={BundleId}",
        ...
        return new WorkerTaskExecutionResult(
            ...
            Message: "iOS uninstall app task completed successfully",
            Log: "iOS uninstall app task execution finished");
    }
    
    private static IosUninstallAppPipelineParameters DeserializePipelineParameters(string? parametersJson)
    {
        if (string.IsNullOrWhiteSpace(parametersJson))
        {
            throw new InvalidOperationException(
                "iOS uninstall app task parameters are empty");
        }
        
        var parameters = JsonSerializer.Deserialize<IosUninstallAppPipelineParameters>(
            parametersJson,
        ...
        {
            throw new InvalidOperationException(
                "iOS uninstall app task parameters are invalid");
        }
        
        if (string.IsNullOrWhiteSpace(parameters.BundleId))
        {
            throw new InvalidOperationException(
                "iOS uninstall app parameter 'bundleId' is required");
        }

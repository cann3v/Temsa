...
public class IosDynamicSessionExecutor(
    IArtifactStorage artifactStorage,
    IFridaScriptProvider scriptProvider,
    IFridaClient fridaClient,
    IDateTimeProvider dateTimeProvider,
    IWorkerDeviceContext deviceContext,
    ILogger<IosDynamicSessionExecutor> logger) : IIosDynamicSessionExecutor
{
    ...
    private readonly ILogger<IosDynamicSessionExecutor> _logger = logger;

    public async Task<IosDynamicSessionExecutionResult> ExecuteAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await events.ReportProgressAsync(
            phase: "preparing",
            message: "Preparing iOS dynamic analysis session",
            percent: 0,
            cancellationToken);

        await using var tempDirectory = TemporaryDirectory.Create("temsa-ios-dynamic");
        var logPath = Path.Combine(tempDirectory.Path, "frida-events.ndjson");
        ...
        await using var fridaSession = await _fridaClient.SpawnAsync(
            new FridaSpawnOptions(
                TargetIdentifier: parameters.BundleId,
        ...
        await events.ReportProgressAsync(
            ...
            message: "iOS application is running with Frida scripts loaded",
            percent: 50,
            cancellationToken);
        ...
        await events.ReportProgressAsync(
            ...
            message: "Uploading raw iOS Frida log artifact",
            percent: 90,
            cancellationToken);

        await using var logStream = File.OpenRead(logPath);

        var objectKey = $"dynamic/ios/{parameters.InputArtifact.Id}/{Guid.NewGuid():N}/frida-events.ndjson";
        ...
        await events.ReportProgressAsync(
            phase: "completed",
            message: "iOS dynamic analysis session completed",
            percent: 100,
            cancellationToken);

        return new IosDynamicSessionExecutionResult(
            Status: "completed",
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (completedTask == stopTask)
        {
            ...
            await control.Events.ReportProgressAsync(
                ...
                message: "iOS dynamic session stop signal received",
                percent: 80,
                cancellationToken);
        }
        else if (completedTask == timeoutTask)
        {
            ...
            await control.Events.ReportProgressAsync(
                ...
                message: "iOS dynamic session timeout reached",
                percent: 80,
                cancellationToken);
        }
